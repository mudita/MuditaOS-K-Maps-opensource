...
import com.mudita.maps.R.color;
...
public class SearchPOIActivity extends OsmandListActivity implements OsmAndCompassListener, OsmAndLocationListener {
	...
	public boolean onCreateOptionsMenu(Menu omenu) {
		...
		showOnMapItem.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				...
				MapActivity.Companion.launchMapActivityMoveToTop(SearchPOIActivity.this);
				return true;
	...
	public Toolbar getClearToolbar(boolean visible) {
		Toolbar tb = findViewById(R.id.poiSplitbar);
		tb.setTitle(null);
		tb.getMenu().clear();
	...
	private void showOptionsMenu(View v) {
		...
		PopupMenu optionsMenu = new PopupMenu(this, v);

		PoiUIFilter f = this.filter;
	...
	private void addFilter(PopupMenu optionsMenu, String value) {
		UiUtilities iconsCache = getMyApplication().getUIUtilities();
	...
	public void updateCompassValue(float value) {
		...
		// }
		View selected = accessibilityAssistant.getFocusedView();
		if (selected != null) {
			try {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		settings.setMapLocationToShow(location.getLatitude(), location.getLongitude(),
				...
				true,
				amenity); //$NON-NLS-1$
		MapActivity.Companion.launchMapActivityMoveToTop(this);
	}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (amenity.getOpeningHours() != null) {
				...
				if (rs != null) {
					...
					timeText.setTextColor(getColor(colorId));
