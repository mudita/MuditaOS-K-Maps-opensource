...
public class RouteCalculationResult {
    ...
    private static final double DISTANCE_THRESHOLD_TO_INTRODUCE_FIRST_AND_LAST_POINTS = 10;
    ...
    protected List<String> missingMaps;
    ...
    public RouteCalculationResult(Exception error) {
        this.error = error;
        this.routingTime = 0;
        this.loadedTiles = 0;
    ...
    public float getCompleteTime() {
        ...
        for (RouteSegmentResult r : getImmutableAllSegments()) {
    ...
    public String toString() {
        return "RouteCalculationResult{" +
                ...
                ", listDistance=" + Arrays.toString(listDistance) +
                ", intermediatePoints=" + Arrays.toString(intermediatePoints) +
                ", routingTime=" + routingTime +
                ", calculateTime=" + calculateTime +
                ", visitedSegments=" + visitedSegments +
                ", loadedTiles=" + loadedTiles +
                ", error='" + error.getMessage() + '\'' +
                ", routeRecalcDistance=" + routeRecalcDistance +
                ", routeVisibleAngle=" + routeVisibleAngle +
                ", appMode=" + appMode +
    ...
    public List<String> getMissingMaps() {
        return missingMaps;
    }
    ...
    private static List<RouteSegmentResult> convertVectorResult(List<RouteDirectionInfo> directions,
        ...
        for (int routeInd = 0; routeInd < list.size(); routeInd++) {
            ...
            if (turn != null) {
                ...
                info.setIntermediatePoint(s.isIntermediatePoint());
    ...
    protected static void addMissingTurnsToRoute(@NonNull List<Location> locations,
        ...
        for (int i = 1; i < locations.size() - 1; i++) {
            ...
            if (delta > 30 && delta < 330) {
                ...
                previousInfo.setDistance(listDistance[previousLocation] - listDistance[i]);
                ...
                if (useLocationTime && current.getTime() > 0 && strictPrevious.getTime() > 0
                    ...
                    float directionSpeed = previousInfo.getDistance() / ((current.getTime() - strictPrevious.getTime()) / 1000f);
        ...
        previousInfo.setDistance(listDistance[previousLocation]);
        Location strictPrevious = locations.get(prevStrictLocation);
        ...
        if (originalDirections.isEmpty()) {
            ...
            for (int i = 0; i <= originalDirections.size() && currentDirection < computeDirections.size(); i++) {
                while (currentDirection < computeDirections.size()) {
                    ...
                    toAdd.setDistance(listDistance[toAdd.routePointOffset] - distanceAfter);
                    if (i < originalDirections.size()) {
        ...
        if (directions != null && directions.size() > 1) {
            for (int i = 1; i < directions.size(); ) {
                ...
                if (r.getTurnType().getValue() == TurnType.C) {
                    ...
                    prev.setAverageSpeed((prev.getDistance() + r.getDistance())
                            / (prev.getDistance() / prev.getAverageSpeed() + r.getDistance() / r.getAverageSpeed()));
                    prev.setDistance(prev.getDistance() + r.getDistance());
                    directions.remove(i);
                } else {
                    i++;
                }
    ...
    private static void updateDirectionsTime(List<RouteDirectionInfo> directions, int[] listDistance) {
        ...
        for (int i = directions.size() - 1; i >= 0; i--) {
            directions.get(i).setDistance(listDistance[directions.get(i).routePointOffset]);
            if (i < directions.size() - 1) {
                directions.get(i).setDistance(directions.get(i).getDistance() - listDistance[directions.get(i + 1).routePointOffset]);
            }
    ...
    public Location getNextLocationFromRouteDirection(RouteDirectionInfo routeDirectionInfo) {
        if (routeDirectionInfo != null && locations != null && routeDirectionInfo.routePointOffset < locations.size()) {
            return locations.get(Math.min(routeDirectionInfo.routePointOffset + 1, locations.size() - 1));
        }
        return null;
    }
    ...
    public List<RouteDirectionInfo> getRouteDirections() {
        if (currentDirectionInfo < directions.size() - 1) {
            if (cacheCurrentTextDirectionInfo != currentDirectionInfo) {
                ...
                List<RouteDirectionInfo> list = directions.subList(currentDirectionInfo + 1, directions.size());
                cacheAgreggatedDirections = new ArrayList<>();
                RouteDirectionInfo p = null;
                for (RouteDirectionInfo i : list) {
                    if (p == null || !i.getTurnType().isSkipToSpeak() || p.isIntermediatePoint()) {
                    ...
                    p.setDistance(p.getDistance()+ i.getDistance());
                    p.setAverageSpeed(p.getDistance() / time);
                    p.afterLeftTime = i.afterLeftTime;
                    p.setIntermediatePoint(i.isIntermediatePoint());
                }
            }
