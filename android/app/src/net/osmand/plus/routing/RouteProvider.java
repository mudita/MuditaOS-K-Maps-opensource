			...
			try {
				...
				if (calcGPXRoute && !params.gpxRoute.calculateOsmAndRoute) {
					...
					if (params.inPublicTransportMode) {
						...
						List<Location> points = MissingMapsChecker.getStartFinishIntermediatePoints(params);
						List<String> missingMaps = MissingMapsChecker.getMissingMaps(params.ctx, points);
						List<Location> pathPoints = MissingMapsChecker.getDistributedPathPoints(points);

						if (!Algorithms.isEmpty(missingMaps)) {
							res = new RouteCalculationResult(new RouteCalculationError.MissingMaps("Additional maps needed"));
							res.missingMaps = MissingMapsChecker.getMissingMaps(params.ctx, pathPoints);
						} else {
							res = findVectorMapsRoute(params, calcGPXRoute);
						}
						if (!MissingMapsChecker.isAnyPointOnWater(pathPoints, params)) {
							params.calculationProgress.missingMaps = MissingMapsChecker.getMissingMaps(params.ctx, pathPoints);
						}
					}
					...
					res = new RouteCalculationResult(new RouteCalculationError.SelectedServiceNotAvailable("Selected route service is not available"));
				}
				if (log.isInfoEnabled()) {
				...
				return new RouteCalculationResult(new RouteCalculationError.RouteNotFound("Failed to find route"));
			}
		}
		return emptyResult();
	}
		...
		try {
			...
			if(complexCtx != null) {
				try {
					...
					log.error("Fast route calculation runtime error, fall back to slow calculation: " + e.getMessage(), e);
			...
			if(result == null || result.isEmpty()) {
				if(ctx.calculationProgress.segmentNotFound == 0) {
					String message = params.ctx.getString(R.string.starting_point_too_far);
					return new RouteCalculationResult(new RouteCalculationError.StartPointTooFarFromRoad(message));
				} else if(ctx.calculationProgress.segmentNotFound == inters.size() + 1) {
					String message = params.ctx.getString(R.string.ending_point_too_far);
					return new RouteCalculationResult(new RouteCalculationError.EndPointTooFarFromRoad(message));
				} else if(ctx.calculationProgress.segmentNotFound > 0) {
					String message = params.ctx.getString(R.string.intermediate_point_too_far, "'" + ctx.calculationProgress.segmentNotFound + "'");
					return new RouteCalculationResult(
							new RouteCalculationError.IntermediatePointTooFarFromRoad(message, ctx.calculationProgress.segmentNotFound - 1)
					);
				}

				if(ctx.calculationProgress.directSegmentQueueSize == 0) {
					String message = "Route can not be found from start point (" + ctx.calculationProgress.distanceFromBegin/1000f + " km)";
					return new RouteCalculationResult(new RouteCalculationError.RouteNotFound(message));
				} else if(ctx.calculationProgress.reverseSegmentQueueSize == 0) {
					String message = "Route can not be found from end point (" + ctx.calculationProgress.distanceFromEnd/1000f + " km)";
					return new RouteCalculationResult(new RouteCalculationError.RouteNotFound(message));
				}
			...
			return new RouteCalculationResult(e);
...
//			int avl = (int) (memoryInfo.availMem / (1 << 20));
			...
			return new RouteCalculationResult(new RouteCalculationError.RouteIsTooComplex("Not enough process memory " + s));
		}
	}

	private RouteCalculationResult applicationModeNotSupported(RouteCalculationParams params) {
		String message = "Application mode '" + params.mode.toHumanString() + "' is not supported.";
		return new RouteCalculationResult(new RouteCalculationError.ApplicationModeNotSupported(message));
	}

	private RouteCalculationResult interrupted() {
		return new RouteCalculationResult(new RouteCalculationError.CalculationCancelled("Route calculation was interrupted / cancelled"));
	}

	private RouteCalculationResult emptyResult() {
		return new RouteCalculationResult(new RouteCalculationError.EmptyRoute("Empty route calculation result"));
	}
				...
				try {
					...
					if(directions.size() > 0) {
						...
						if (distanceToEnd.length > last.routePointOffset && distanceToEnd.length > offset) {
							...
							last.setDistance(Math.round(lastDistanceToEnd - currentDistanceToEnd));
						}
					} 
	...
	protected RouteCalculationResult findBROUTERRoute(RouteCalculationParams params) throws MalformedURLException,
		...
		if (brouterService == null) {
			return new RouteCalculationResult(new RouteCalculationError.SelectedServiceNotAvailable("BRouter service is not available"));
		}
		try {
			...
			if (!(isZ64Encoded || gpxMessage.startsWith("<"))) {
				return new RouteCalculationResult(new RouteCalculationError.SelectedServiceNotAvailable(gpxMessage));
			}
			...
			return new RouteCalculationResult(new RouteCalculationError.SelectedServiceNotAvailable("Exception calling BRouter: " + e.getMessage()));
		}
