...
public class VoiceRouter {
	...
	public boolean isMute() {
		return !app.settingsRepository.getSoundEnabled();
	}
	...
	private void registerMuteChangeListener() {
		app.settingsRepository.registerOnSoundChangedListener(soundEnabled -> {
			if (!soundEnabled) {
				player.stop();
			}
	...
	private boolean isTargetPoint(NextDirectionInfo info) {
		...
		boolean target = info == null || info.directionInfo == null
				|| info.directionInfo.getDistance() == 0;
		return in || target;
	}
