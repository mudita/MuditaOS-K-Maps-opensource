...
public class DownloadValidationManager {
	...
	public void startDownload(@NonNull Context context, IndexItem... items) {
		downloadFilesWithAllChecks(context, items);
	}
	...
	private void downloadFilesWithAllChecks(@NonNull Context context, IndexItem... items) {
		downloadFilesCheck_1_Internet(context, items);
	}
	...
	private void downloadFilesCheck_1_Internet(@NonNull Context context, IndexItem[] items) {
		if (settings.isWifiConnected()) {
			downloadFilesCheck_2_ValidateSpace(context, items);
		} else if (settings.isInternetConnectionAvailable()) {
			if (DOWNLOAD_MOBILE_INTERNET_CONFIRMED) {
				downloadFilesCheck_2_ValidateSpace(context, items);
			} else {
	...
	private void downloadFilesCheck_2_ValidateSpace(@NonNull Context context, IndexItem... items) {
		long szChangeLong = 0;
		long szMaxTempLong = 0;
		int i = 0;
		for (IndexItem es : downloadThread.getCurrentDownloadingItems()) {
