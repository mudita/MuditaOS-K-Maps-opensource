...
abstract class SettingsMapPointsStorage {
	...
	public boolean insertPoint(double latitude, double longitude, PointDescription historyDescription, int index) {
		...
		String pointDescription = PointDescription.serializeToString(historyDescription);
		if (index < points.size()) points.set(index, latLon); else points.add(latLon);
		if (index < descriptions.size()) descriptions.set(index, pointDescription); else descriptions.add(pointDescription);
