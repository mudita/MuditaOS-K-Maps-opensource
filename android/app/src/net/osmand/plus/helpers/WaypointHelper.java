			...
			if (type == POI) {
				...
				if (((TargetPoint) point).start) {
					if (app.getTargetPointsHelper().getPointToStart() == null) {
						...
						return iconsCache.getIcon(com.mudita.map.common.R.drawable.ic_from_start, 0);
					}
				} else if (((TargetPoint) point).intermediate) {
					return iconsCache.getIcon(com.mudita.map.common.R.drawable.ic_intermediate_flag, 0);
				} else {
					return iconsCache.getIcon(com.mudita.map.common.R.drawable.ic_finish_flag, 0);
				}
				...
				if (typeString.equals("SPEED_CAMERA")) {
					return AppCompatResources.getDrawable(uiCtx, com.mudita.map.common.R.drawable.mx_mudita_default);
