...
public class OsmandRenderer {
	...
	public Map<LatLon, Integer> displayedAmenities = new HashMap<>();
	...
	void drawObject(RenderingContext rc,  Canvas cv, RenderingRuleSearchRequest req,
			...
			for (int i = 0; i < array.size(); i++) {
				...
				if (array.get(i).objectType == 3) {
					...
					drawPoint(mObj, req, rc, pair, array.get(i).typeInd == 0);
				}
	...
	private void drawIconsOverCanvas(RenderingContext rc, Canvas cv) {
		displayedAmenities.clear();
		// 1. Sort text using text order
		rc.iconsToDraw.sort(Comparator.comparingInt(object -> object.iconOrder));
			...
			if (icon.resId != null) {
				...
				if (ico != null) {
					if (icon.y >= 0 && icon.y < rc.height && icon.x >= 0 && icon.x < rc.width) {
						int visibleWidth = icon.iconSize >= 0 ? (int) icon.iconSize : ico.getIntrinsicWidth();
						int visibleHeight = icon.iconSize >= 0 ? (int) icon.iconSize : ico.getIntrinsicHeight();
						...
						if (visibleHeight > 0 && visibleWidth > 0) {
							visibleRect = calculateRect(rc, icon, visibleWidth, visibleHeight);
						...
						if (!intersects) {
							...
							if (shield != null) {
								...
								cv.restore();
								displayedAmenities.put(icon.latLon, (int) Math.max(shieldRf.width(), shieldRf.height()));
							} else {
								displayedAmenities.put(icon.latLon, Math.max(visibleWidth, visibleHeight));
							}
							cv.save();
...
//		reverse(linesResArray.begin(), linesResArray.end());
	...
	private void drawPolygon(BinaryMapDataObject obj, RenderingRuleSearchRequest render, Canvas canvas, RenderingContext rc, TagValuePair pair, 
		...
		if (polygonInnerCoordinates != null && path != null) {
			...
			for (int[] polygonInnerCoordinate : polygonInnerCoordinates) {
				for (int i = 0; i < polygonInnerCoordinate.length; i += 2) {
					PointF p = calcPoint(polygonInnerCoordinate[i], polygonInnerCoordinate[i + 1], rc);
					if (i == 0) {
						path.moveTo(p.x, p.y);
					} else {
						path.lineTo(p.x, p.y);
		...
		if (path != null && len > 0) {
			...
			boolean ignorePointArea = render.getIntPropertyValue(render.ALL.R_IGNORE_POLYGON_AS_POINT_AREA) != 0;
			if (area > MAX_V_AREA || ignorePointArea) {
				textRenderer.renderText(obj, render, rc, pair, obj.getLabelX(), obj.getLabelY(), null,
						null, false);
			}
		}
	}
	...
	private void drawPoint(BinaryMapDataObject obj, RenderingRuleSearchRequest render, RenderingContext rc, TagValuePair pair, boolean renderText) {
		if(render == null || pair == null){
			return;
		}
		...
		rc.visible++;
		PointF iconCoordinates = calcPoint(obj.getLabelX(), obj.getLabelY(), rc);
		if(resId != null){
			IconDrawInfo ico = new IconDrawInfo();
			ico.x = iconCoordinates.x;
			ico.y = iconCoordinates.y;
			...
			ico.latLon = new LatLon(MapUtils.get31LatitudeY(obj.getLabelY()), MapUtils.get31LongitudeX(obj.getLabelX()));
			rc.iconsToDraw.add(ico);
		}
		if (renderText) {
			textRenderer.renderText(obj, render, rc, pair, iconCoordinates.x, iconCoordinates.y,
					null, null, resId != null);
		}
...
//			}
		...
		if (path != null) {
			...
			if (textPoints != null) {
				textRenderer.renderText(obj, render, rc, pair, xMid, yMid, path, textPoints, false);
			}
		}
	}
