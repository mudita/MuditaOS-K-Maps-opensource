...
//		drawTestBox(cv, text.bounds, text.pathRotate, text.text);
		...
		if (render.search(RenderingRulesStorage.TEXT_RULES)) {
			if (render.getFloatPropertyValue(render.ALL.R_TEXT_SIZE) > 0) {
				...
				if (text.drawOnPath == null) {
					...
					// Shift the text down. Fixes the POI labels overlapping their icon shields
					if (isPoiText) {
						text.centerY += (paintText.descent() - paintText.ascent()) / 2;
					}
	...
	public void renderText(BinaryMapDataObject obj, RenderingRuleSearchRequest render,
	                       ...
	                       float yMid, Path path, PointF[] points, boolean isPoiText) {
		TIntObjectHashMap<String> map = obj.getObjectNames();
		if (map != null) {
			ArrayList<String> filteredNameTags = new ArrayList<>();
			ArrayList<String> filteredNames = new ArrayList<>();
			final String preferredNameTag = TAG_NAME + rc.preferredLocale;
			map.forEachEntry((tag, name) -> {
                if (name != null && !name.trim().isEmpty()) {
                    boolean isName = tag == obj.getMapIndex().nameEncodingType;
                    String nameTag = isName ? "" : obj.getMapIndex().decodeType(tag).tag;
					if (!nameTag.contains(TAG_NAME) || nameTag.equals(preferredNameTag)) {
						int index = nameTag.equals(preferredNameTag) ? 0 : filteredNameTags.size();
                        filteredNameTags.add(index, nameTag);
                        filteredNames.add(index, name);
					}
                }
                return true;
            });
			for (int i =  0; i < filteredNameTags.size(); i++) {
                createTextDrawInfo(obj, render, rc, pair, xMid, yMid, path, points,
						filteredNames.get(i), filteredNameTags.get(i), isPoiText);
			}
		}
	}
