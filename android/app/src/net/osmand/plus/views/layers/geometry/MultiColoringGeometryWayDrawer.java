...
import static com.mudita.map.common.utils.ConstantsKt.MAX_ZOOM_LEVEL;
...
public class MultiColoringGeometryWayDrawer<T extends MultiColoringGeometryWayContext>
    ...
    private static final int BORDER_TYPE_ZOOM_THRESHOLD = MAX_ZOOM_LEVEL + MapTileLayer.OVERZOOM_IN;

    private final Float pathEndCircleRadius;
    ...
    public MultiColoringGeometryWayDrawer(T context) {
        ...
        pathEndCircleRadius = context.ctx.getResources().getDimension(R.dimen.route_line_end_circle_radius);
    }
    ...
    protected void drawFullBorder(@NonNull Canvas canvas, int zoom, @NonNull List<DrawPathData> pathsData) {
        if (zoom < BORDER_TYPE_ZOOM_THRESHOLD) {
    ...
    protected void drawFullBorder(@NonNull VectorLinesCollection collection, int baseOrder,
        ...
        if (!pathsData.isEmpty() && zoom < BORDER_TYPE_ZOOM_THRESHOLD) {
    ...
    @Override
    protected void drawRouteEdgeBottomLayerDecorationBorder(@NonNull Canvas canvas, int zoom, @NonNull DrawPathData pathData) {
        if (pathData.style.color != 0) {
            PointF point = pathData.end;
            canvas.drawCircle(point.x, point.y, pathEndCircleRadius, getContext().getBorderPaint());
        }
    }

    @Override
    protected void drawRouteEdgeTopLayerDecorationBorder(@NonNull Canvas canvas, int zoom, @NonNull DrawPathData pathData) {
        if (pathData.style.color != 0) {
            PointF point = pathData.end;
            canvas.drawCircle(point.x, point.y, pathEndCircleRadius, getContext().getCustomPaint());
            Paint borderPaint = getContext().getBorderPaint();
            Paint.Style originalStyle = borderPaint.getStyle();
            borderPaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(point.x, point.y, pathEndCircleRadius, borderPaint);
            borderPaint.setStyle(originalStyle);
        }
    }
