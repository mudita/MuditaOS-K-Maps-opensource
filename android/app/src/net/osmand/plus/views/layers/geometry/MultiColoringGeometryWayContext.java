...
public abstract class MultiColoringGeometryWayContext extends GeometryWayContext {
	...
	public MultiColoringGeometryWayContext(@NonNull Context ctx, float density) {
		...
		Bitmap specialArrowBitmap = AndroidUtils.drawableToBitmap(ContextCompat.getDrawable(ctx, com.mudita.map.common.R.drawable.mm_special_arrow_up));
		if (specialArrowBitmap != null) {
			specialArrowBitmap = AndroidUtils.scaleBitmap(specialArrowBitmap,
					(specialArrowBitmap.getWidth()), (specialArrowBitmap.getHeight()), false);
		}
	...
	public float getBorderOutlineWidth() {
		return ctx.getResources().getDimensionPixelSize(R.dimen.route_line_outline_width);
	}
