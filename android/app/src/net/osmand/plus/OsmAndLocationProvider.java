...
public class OsmAndLocationProvider implements SensorEventListener {
	...
	private static final long CURRENT_LOCATION_TIMEOUT = 5000; // 5 seconds
	...
	public AtomicBoolean isStaleLocationInitialized = new AtomicBoolean(false);
	...
	public void removeLocationListener(@NonNull OsmAndLocationListener listener) {
        // This prevents a CME when the listener is removed right from `OsmAndLocationListener.updateLocation` callback.
        app.runInUIThread(() -> {
            synchronized (locationListeners) {
	...
	private void setLocation(net.osmand.Location location) {
		...
		isStaleLocationInitialized.set(true);
	...
	public boolean isStaleLocationInitialized() {
		return isStaleLocationInitialized.get();
	}
	...
	@Nullable
	public net.osmand.Location getCurrentLocation() {
		if (System.currentTimeMillis() - lastTimeLocationFixed > CURRENT_LOCATION_TIMEOUT) {
			return null;
		}
		return location;
	}
	...
	public boolean checkGPSEnabled() {
		Pair<Boolean, Boolean> locationAvailability = getGPSAvailability();
		boolean gpsEnabled = locationAvailability.first;
		boolean networkEnabled = locationAvailability.second;

        return gpsEnabled || networkEnabled;
    }

	public Pair<Boolean, Boolean> getGPSAvailability() {
		...
		return new Pair<>(gpsEnabled, networkEnabled);
	}
	...
	public static boolean hasLocationAccess(@NonNull Context context) {
		LocationManager lm = (LocationManager) context.getSystemService(Context.LOCATION_SERVICE);
		return lm.isProviderEnabled(LocationManager.GPS_PROVIDER) && isLocationPermissionAvailable(context);
	}
