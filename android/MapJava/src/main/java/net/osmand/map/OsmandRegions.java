...
public class OsmandRegions {
   ...
   private List<RegionsIndexedListener> regionsIndexedListeners = new ArrayList<>();

   private boolean initialRegionsIndexComplete = false;
   ...
   public BinaryMapIndexReader prepareFile(String fileName) throws IOException {
      ...
      final ResultMatcher<BinaryMapDataObject> resultMatcher = new ResultMatcher<>() {
            ...
            if (boundary) {
               ...
               if (region != null) {
                  addPolygonToRegionIfValid(object, region);
                   ...
                   List<BinaryMapDataObject> unattachedMapObjects = unattachedBoundaryMapObjectsByRegions.computeIfAbsent(fullRegionName, k -> new ArrayList<>());
      ...
      initialRegionsIndexComplete = true;
      for (RegionsIndexedListener listener : regionsIndexedListeners) {
         listener.onRegionsIndexed();
      }
      return reader;
   }

   public boolean isInitialRegionsIndexComplete() {
      return initialRegionsIndexComplete;
   }
   ...
   private List<BinaryMapDataObject> getCountries(int lx, int rx, int ty, int by, final boolean checkCenter) throws IOException {
      HashSet<String> set = new HashSet<String>(quadTree.queryInBox(new QuadRect(lx, ty, rx, by), new ArrayList<String>()));
   ...
   public void registerRegionsIndexedListener(RegionsIndexedListener listener) {
      regionsIndexedListeners.add(listener);
   }

   public void unregisterRegionsIndexedListener(RegionsIndexedListener listener) {
      regionsIndexedListeners.remove(listener);
   }

   public interface RegionsIndexedListener {
      void onRegionsIndexed();
   }
}