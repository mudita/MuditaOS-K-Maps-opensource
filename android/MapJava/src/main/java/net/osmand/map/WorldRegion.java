...
public class WorldRegion implements Serializable {
   ...
   public static final String PORTUGAL_REGION_ID = "europe_portugal";
   ...
   public String getLocaleName() {
      if (!Algorithms.isEmpty(regionNameLocale)) {
         return Algorithms.capitalizeFirstLetter(regionNameLocale);
      }
      if (!Algorithms.isEmpty(regionNameEn)) {
         return Algorithms.capitalizeFirstLetter(regionNameEn);
      }
      if (!Algorithms.isEmpty(regionName)) {
         return Algorithms.capitalizeFirstLetter(regionName);
      }

      // Example: Europe_Gb_England_Channel-islands -> Channel Islands
      String[] name = this.regionFullName.split("_");
      return capitalize(name[name.length-1].replace("-", " "));
   }
